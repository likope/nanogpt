...
Then I've run the same architecture, scaled up to 10M parameters, this model memorizes the patterns and overfits, with a 1.55 in validation set but 1.15 in training set, then I've updated the model with a dropout method, which turns off a random percentage of neurons in training to prevent the unstable connections used for memorization, and with 20% dropout the model reached 1.499 loss on the validation set with 1.22 on the training set; then I've run the model with 30% dropout but reached 1.51 loss in the validation set, I turned off so many neurons that the model wasn't capable of correctly learning.
To detect overfitting and underfitting, I built a function that calculates the loss in the training and validation set, if the loss decreases in the training set but increases in the validation set it's overfitting, else if both stay high, it's underfitting.
